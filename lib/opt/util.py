...
def get_local_session_key(drkeys):
    ...
    This key is need to initialize the PVF when checking the validity of the packet.
...
def get_intermediate_session_keys(drkeys):
    ...
    Thus, during verification of a received packet, simply iterate through the list.
...
class OPTStore(object):
    ...
    def __init__(self):
        self._fingerprint_map = dict()  # mapping {session_id -> [(data hash, pvf)]} used to verify
    ...
    def insert_packet(self, pkt):
        ...
        if ext_hdr:
            if ext_hdr.session_id in self._fingerprint_map:
                self._fingerprint_map[ext_hdr.session_id].append((OPTExt.compute_data_hash(pkt.get_payload()), ext_hdr.pvf))
            else:
                self._fingerprint_map[ext_hdr.session_id] = [(OPTExt.compute_data_hash(pkt.get_payload()), ext_hdr.pvf)]
    ...
    def _validate_fingerprint(fingerprint, drkeys):
        ...
        pvf = OPTExt.compute_initial_pvf(get_local_session_key(drkeys), fingerprint[0])
...
class DRKeys(object):
    ...
    def __init__(self, src_key, intermediate_keys, dst_key, is_source):
        ...
        :type src_key: bytes
        :param intermediate_keys: the keys of the ASes. Starting from the source.
        :type intermediate_keys: [bytes]
        :param dst_key: The key of the non-initiator end-host
        :type dst_key: bytes
        :param is_source: indicate if the holder of this DRKeys object is the initiator of the DRKey exchange.
        :type is_source: bool
        """
    ...
    def from_bytes_list(cls, bytes_list, src_key):
        ...
        This method is used by the sciond and thus makes some special assumptions.
